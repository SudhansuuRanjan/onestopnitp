import {
    View, Text, ScrollView, StyleSheet, Image, TouchableOpacity, Linking, Modal, TextInput, ActivityIndicator,
    Keyboard, Pressable, Button, Alert
} from 'react-native'
...
function AllPostScreen({ navigation }) {
    ...
    useEffect(() => {
        postRef.where('type', '==', 'Lost').onSnapshot((querySnapshot) => {
            const posts = [];
...
function UserPostScreen() {
    ...
    const validateInput = () => {
        ...
        if (email !== "" || whatsappNo !== "" || mobileNo !== "" && description !== "") {
            temp = true;
        }
        return temp;
    }
